...
1. Connect via FTP client (FileZilla, Cyberduck, etc.)
   - Host: `ftp.teammurphy.rocks` or your server IP
...
3. **Fill in the form:**
   ...
   Application URL: teammurphy.rocks (or use subdomain like api.teammurphy.rocks)
...
2. Edit line 6:
   ...
   **If using subdomain (recommended):**
   ```javascript
   const API_BASE_URL = 'https://api.teammurphy.rocks/api';
   ```

### Step 7: Configure Reverse Proxy (If using main domain)

If you want both frontend and backend on `teammurphy.rocks`:
...
# or
curl https://api.teammurphy.rocks/health
```
...
- Application URL: `api.teammurphy.rocks`
- Application root: `storm-scout`
...
- Frontend: `https://teammurphy.rocks`
- Backend API: `https://api.teammurphy.rocks/api`
...
- Test backend directly: `curl https://teammurphy.rocks/api/sites`
