...
**Live URL:** https://your-domain.example.com
...
- [ ] Update API base URL in `frontend/js/api.js`
  - Change from `http://localhost:3000` to `https://your-domain.example.com`
- [ ] Deploy frontend files to server
  ```bash
  rsync -avz -e "ssh -p REDACTED_PORT" frontend/ REDACTED_USER@66.29.148.111:~/public_html/
  ```
- [ ] Test frontend loads at https://your-domain.example.com
...
rsync -avz -e "ssh -p REDACTED_PORT" \
  ...
  --exclude='.env' \
  --exclude='*.db*' \
  backend/ REDACTED_USER@66.29.148.111:~/storm-scout/
...
cd /Users/mmurphy/strom-scout
rsync -avz -e "ssh -p REDACTED_PORT" \
  frontend/ REDACTED_USER@66.29.148.111:~/public_html/
```
...
- **SSH:** `ssh stormscout` (port REDACTED_PORT)
- **cPanel:** https://***REDACTED_HOST***:2083
- **API:** https://your-domain.example.com
