...
const OfficeAggregator = require('./aggregation');
...
describe('calculateUrgency', () => {
    test('Extreme severity scores higher than Minor', () => {
        ...
        expect(OfficeAggregator.calculateUrgency(extreme)).toBeGreaterThan(OfficeAggregator.calculateUrgency(minor));
    });

    test('NEW action adds bonus', () => {
        const withNew = makeAdvisory({ vtec_action: 'NEW' });
        const withCon = makeAdvisory({ vtec_action: 'CON' });
        expect(OfficeAggregator.calculateUrgency(withNew)).toBeGreaterThan(OfficeAggregator.calculateUrgency(withCon));
    });

    test('UPG action adds higher bonus than NEW', () => {
        const withUpg = makeAdvisory({ vtec_action: 'UPG' });
        const withNew = makeAdvisory({ vtec_action: 'NEW' });
        expect(OfficeAggregator.calculateUrgency(withUpg)).toBeGreaterThan(OfficeAggregator.calculateUrgency(withNew));
    });
    ...
    test('soon-expiring alert gets bonus', () => {
        ...
        expect(OfficeAggregator.calculateUrgency(soonExpire)).toBeGreaterThan(
            OfficeAggregator.calculateUrgency(laterExpire)
        );
    });
});
...
describe('getSummaryStats', () => {
    test('computes correct counts', () => {
        ...
        const offices = [{ highest_severity: 'Extreme' }, { highest_severity: 'Moderate' }];
