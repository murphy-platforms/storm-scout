...
## ✅ Completed
- MySQL database created: `mwqtiakilx_stormscout`
- MySQL user created: `mwqtiakilx_stormscout` with ALL PRIVILEGES
...
cp -r ~/teammurphy.rocks/stormscout/* ~/storm-scout/
...
1. Verify database exists: `mwqtiakilx_stormscout`
...
```bash
mysql -u mwqtiakilx_stormscout -p mwqtiakilx_stormscout -e "SHOW TABLES;"
```

### View Sites Count
```bash
mysql -u mwqtiakilx_stormscout -p mwqtiakilx_stormscout -e "SELECT COUNT(*) FROM sites;"
```
...
cd ~/storm-scout
source ~/nodevenv/storm-scout/20/bin/activate
mysql -u mwqtiakilx_stormscout -p mwqtiakilx_stormscout -e "DROP DATABASE mwqtiakilx_stormscout; CREATE DATABASE mwqtiakilx_stormscout;"
npm run init-db
npm run seed-db
```
...
- CORS origin: https://teammurphy.rocks
