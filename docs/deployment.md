...
- `ssh stormscout` instead of `ssh -p 21098 user@66.29.148.111`
...
scp -P 21098 frontend/index.html stormscout:~/public_html/index.html

# Deploy multiple HTML files
scp -P 21098 frontend/*.html stormscout:~/public_html/

# Deploy JavaScript files
scp -P 21098 frontend/js/*.js stormscout:~/public_html/js/

# Deploy specific JS component
scp -P 21098 frontend/js/update-banner.js stormscout:~/public_html/js/

# Deploy CSS
scp -P 21098 frontend/css/*.css stormscout:~/public_html/css/
```
...
curl https://teammurphy.rocks/api/status/overview

# 3. Test specific API endpoints
curl https://teammurphy.rocks/api/advisories/active | jq .
curl https://teammurphy.rocks/api/sites | jq .
...
curl https://teammurphy.rocks/api/status/overview | jq '.last_updated'
...
scp -P 21098 stormscout:~/backups/manual_*.sql.gz ~/local-backups/
