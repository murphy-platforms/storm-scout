...
**Vanilla JavaScript (no TypeScript)** — Storm Scout uses vanilla ES2020+ JavaScript with no TypeScript and no build step. This is a deliberate choice driven by the project's AI-authored development model. TypeScript's primary value — helping human developers catch type errors, navigate unfamiliar codebases, and maintain consistency across teams — doesn't apply when all code is generated by AI tools (Claude, Warp Oz). The AI agents don't need type annotations to understand the codebase or avoid type mistakes. Vanilla JS eliminates build tooling complexity, keeps the frontend servable as static files, and enables faster iteration cycles with AI assistants.
