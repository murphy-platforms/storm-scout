...
With SSH access to production (`ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com`), we can perform **comprehensive validation** beyond what API calls alone provide:
...
```bash
ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com "cd storm-scout && cat stderr.log"
```
...
ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com "cd storm-scout && ls -lt .last-ingestion.json* | head -10"

# Or monitor in real-time
ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com "watch -n 60 'cat storm-scout/.last-ingestion.json'"
```
...
ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com "cd storm-scout && cat package.json | grep version"
...
ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com "cd storm-scout && grep -o '^[A-Z_]*=' .env | sort"
```
...
ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com "mysql -u REDACTED_USER_stormsc -p'PASSWORD' ***REDACTED*** -e 'DESCRIBE advisories;'"

# Check for indexes
ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com "mysql -u REDACTED_USER_stormsc -p'PASSWORD' ***REDACTED*** -e 'SHOW INDEXES FROM advisories;'"

# Check constraints
ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com "mysql -u REDACTED_USER_stormsc -p'PASSWORD' ***REDACTED*** -e 'SELECT * FROM information_schema.TABLE_CONSTRAINTS WHERE TABLE_NAME=\"advisories\";'"
```
...
ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com "tail -f ~/logs/access.log" | grep "/api/"

# Monitor database queries (if slow query log enabled)
ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com "tail -f ~/logs/mysql-slow.log"

# Monitor system resources
ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com "top -b -n 1 | head -20"
```
...
ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com "ls -lh ~/backups/*.sql 2>/dev/null || echo 'No backups found'"

# Check cPanel backup schedule
ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com "ls -lh ~/backup-* 2>/dev/null"
```
...
ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com "passenger-status 2>/dev/null || echo 'Passenger CLI not available (managed via cPanel)'"

# Alternative: Check Node process
ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com "ps aux | grep 'node.*storm-scout'"
```
...
1. **BUG-PROD-001: Severity "Unknown" in database**
   ...
   ssh -p REDACTED_PORT REDACTED_USER@your-domain.example.com "mysql -u REDACTED_USER_stormsc -p'PASSWORD' ***REDACTED*** -e \"UPDATE advisories SET severity='Minor' WHERE severity='Unknown';\""
...
# Then search for: stormscout, teammurphy, or REDACTED_USER
```
