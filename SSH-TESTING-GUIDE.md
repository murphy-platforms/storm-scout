...
With SSH access to production (`ssh -p 21098 mwqtiakilx@teammurphy.rocks`), we can perform **comprehensive validation** beyond what API calls alone provide:
...
```bash
ssh -p 21098 mwqtiakilx@teammurphy.rocks "cd storm-scout && cat stderr.log"
```
...
ssh -p 21098 mwqtiakilx@teammurphy.rocks "cd storm-scout && ls -lt .last-ingestion.json* | head -10"

# Or monitor in real-time
ssh -p 21098 mwqtiakilx@teammurphy.rocks "watch -n 60 'cat storm-scout/.last-ingestion.json'"
```
...
ssh -p 21098 mwqtiakilx@teammurphy.rocks "cd storm-scout && cat package.json | grep version"
...
ssh -p 21098 mwqtiakilx@teammurphy.rocks "cd storm-scout && grep -o '^[A-Z_]*=' .env | sort"
```
...
ssh -p 21098 mwqtiakilx@teammurphy.rocks "mysql -u mwqtiakilx_stormsc -p'PASSWORD' mwqtiakilx_stormscout -e 'DESCRIBE advisories;'"

# Check for indexes
ssh -p 21098 mwqtiakilx@teammurphy.rocks "mysql -u mwqtiakilx_stormsc -p'PASSWORD' mwqtiakilx_stormscout -e 'SHOW INDEXES FROM advisories;'"

# Check constraints
ssh -p 21098 mwqtiakilx@teammurphy.rocks "mysql -u mwqtiakilx_stormsc -p'PASSWORD' mwqtiakilx_stormscout -e 'SELECT * FROM information_schema.TABLE_CONSTRAINTS WHERE TABLE_NAME=\"advisories\";'"
```
...
ssh -p 21098 mwqtiakilx@teammurphy.rocks "tail -f ~/logs/access.log" | grep "/api/"

# Monitor database queries (if slow query log enabled)
ssh -p 21098 mwqtiakilx@teammurphy.rocks "tail -f ~/logs/mysql-slow.log"

# Monitor system resources
ssh -p 21098 mwqtiakilx@teammurphy.rocks "top -b -n 1 | head -20"
```
...
ssh -p 21098 mwqtiakilx@teammurphy.rocks "ls -lh ~/backups/*.sql 2>/dev/null || echo 'No backups found'"

# Check cPanel backup schedule
ssh -p 21098 mwqtiakilx@teammurphy.rocks "ls -lh ~/backup-* 2>/dev/null"
```
...
ssh -p 21098 mwqtiakilx@teammurphy.rocks "passenger-status 2>/dev/null || echo 'Passenger CLI not available (managed via cPanel)'"

# Alternative: Check Node process
ssh -p 21098 mwqtiakilx@teammurphy.rocks "ps aux | grep 'node.*storm-scout'"
```
...
1. **BUG-PROD-001: Severity "Unknown" in database**
   ...
   ssh -p 21098 mwqtiakilx@teammurphy.rocks "mysql -u mwqtiakilx_stormsc -p'PASSWORD' mwqtiakilx_stormscout -e \"UPDATE advisories SET severity='Minor' WHERE severity='Unknown';\""
...
- [ ] Verify ingestion: `curl https://teammurphy.rocks/api/status/overview | jq '.data.last_updated'`
...
security find-generic-password -s "mwqtiakilx_stormscout" -w
