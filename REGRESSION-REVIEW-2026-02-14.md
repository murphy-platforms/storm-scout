...
**Environment**: Production (https://teammurphy.rocks)  
...
- [ ] Classic UI loads: https://teammurphy.rocks
- [ ] Beta UI loads: https://teammurphy.rocks/beta
...
curl -s https://teammurphy.rocks/api/health

# 2. Get overview data
curl -s https://teammurphy.rocks/api/status/overview | jq

# 3. Get active advisories count
curl -s https://teammurphy.rocks/api/advisories/active | jq '.count'
...
fetch('https://teammurphy.rocks/api/invalid').then(r => console.log('Error handling:', r.status));
```
