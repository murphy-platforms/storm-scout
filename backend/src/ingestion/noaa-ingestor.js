...
async function ingestNOAAData() {
    ...
    try {
        ...
        for (const office of offices) {
            ...
            if (office.ugc_codes) {
                try {
                    const ugcCodes =
                        typeof office.ugc_codes === 'string' ? JSON.parse(office.ugc_codes) : office.ugc_codes;
        ...
        for (const alert of noaaAlerts) {
            ...
            if (matchedSites.size === 0) {
                for (const state of states) {
                    ...
                    stateOffices.filter((o) => !o.ugc_codes).forEach((o) => matchedSites.add(o));
                }
            }
            ...
            for (const [officeId, advisories] of officeAdvisories.entries()) {
                ...
                for (const advisory of advisories) {
                    try {
                        ...
                        await connection.query('INSERT IGNORE INTO alert_types (type_name, category) VALUES (?, ?)', [
    ...
    `);
        ...
        if (processedExternalIds.length > 0) {
            ...
            if (activeTotal > 0 && processedExternalIds.length < activeTotal * 0.1) {
                console.warn(
                    `⚠️  WARNING: Only ${processedExternalIds.length} IDs processed vs ${activeTotal} active advisories — possible partial NOAA response. Skipping expiry.`
                );
            } else {
    ...
    `);
        ...
        console.log(
            `Observations updated: ${observationResult.updated}/${observationResult.total} sites (${observationResult.uniqueStations} unique stations)`
        );
        if (totalFailed > 0) {
            console.warn(
                `⚠️  Partial failures: ${advisoriesFailed} advisory write(s) failed, ${statusesFailed} status update(s) failed`
            );
    ...
    for (const [stationId, stationSites] of stationToSites.entries()) {
        try {
            ...
            if (data.observed_at) {
                ...
                if (ageMinutes > 120) {
                    console.warn(
                        `  ⚠️  Station ${stationId}: observation is ${Math.round(ageMinutes)} min old (stale)`
                    );
                }
            }
