...
async function importCsv(csvPath) {
    ...
    for await (const line of rl) {
        ...
        if (missingValues.length > 0) {
            console.warn(
                `  Line ${lineNumber}: Skipping row — missing required fields: ${missingValues.join(', ')} (zip: ${row.zip || 'unknown'})`
            );
            skipped++;
            continue;
        }
        ...
        if (!/^[A-Z]{2}$/.test(state)) {
            console.warn(
                `  Line ${lineNumber}: Skipping row — state "${row.state}" is not a 2-letter code (zip: ${zip})`
            );
            skipped++;
            continue;
        }
        ...
        if (row.ugc_codes && row.ugc_codes.trim()) {
            ...
            try {
                ...
                if (Array.isArray(parsed)) {
                    ...
                    console.warn(
                        `  Line ${lineNumber}: ugc_codes is not a JSON array for zip ${zip} — skipping ugc_codes`
                    );
                }
            } catch {
                // Try comma-separated
