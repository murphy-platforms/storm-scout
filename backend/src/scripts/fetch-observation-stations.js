...
async function fetchObservationStations() {
    ...
    const [offices] = await db.query(
        'SELECT id, site_code, name, city, state, latitude, longitude, observation_station FROM offices ORDER BY state, city'
    );
    ...
    for (let i = 0; i < offices.length; i++) {
        ...
        if (office.observation_station && !process.argv.includes('--force')) {
            console.log(
                `${progress} ${office.site_code} (${office.city}, ${office.state}) - Already mapped to ${office.observation_station}, skipping`
            );
            skipped++;
            continue;
        }
        ...
        try {
            const stations = await getObservationStations(parseFloat(office.latitude), parseFloat(office.longitude));
            ...
            console.log(
                `${progress} ${office.site_code} (${office.city}, ${office.state}) -> ${stationId} (${stationName})`
            );

            if (!isDryRun) {
                await db.query('UPDATE offices SET observation_station = ? WHERE id = ?', [stationId, office.id]);
            }

            updated++;
        } catch (error) {
            console.error(
                `${progress} ${office.site_code} (${office.city}, ${office.state}) - ERROR: ${error.message}`
            );
