...
async function main() {
    ...
    for (const r of results) {
        ...
        const diffs = [];
        if (Math.abs(entry.latitude - current.latitude) > 0.001)
            diffs.push(`lat: ${current.latitude} → ${entry.latitude}`);
        if (Math.abs(entry.longitude - current.longitude) > 0.001)
            diffs.push(`lon: ${current.longitude} → ${entry.longitude}`);
        ...
        if (JSON.stringify(entry.ugc_codes) !== JSON.stringify(current.ugc_codes))
            diffs.push(`ugc: ${JSON.stringify(current.ugc_codes)} → ${JSON.stringify(entry.ugc_codes)}`);
    ...
    const changed = verified.filter((v, i) => {
        ...
        return (
            Math.abs(v.latitude - c.latitude) > 0.001 ||
