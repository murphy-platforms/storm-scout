...
```bash
# Upload migration
scp -P 21098 backend/src/data/migrations/add-vtec-event-id-and-action.sql stormscout:~/

# Execute migration
ssh stormscout "mysql -h localhost -u mwqtiakilx_stormscout -p'hUnva2-qusvot-cekpuf' mwqtiakilx_stormscout < ~/add-vtec-event-id-and-action.sql"
```
...
scp -P 21098 backend/src/ingestion/utils/normalizer.js stormscout:~/storm-scout/src/ingestion/utils/

# Deploy advisory model (uses event ID for deduplication)
scp -P 21098 backend/src/models/advisory.js stormscout:~/storm-scout/src/models/

# Deploy scripts
scp -P 21098 backend/src/scripts/backfill-vtec-event-id.js stormscout:~/storm-scout/src/scripts/
scp -P 21098 backend/src/scripts/cleanup-event-id-duplicates.js stormscout:~/storm-scout/src/scripts/
```
...
```bash
# Check schema
ssh stormscout "mysql -h localhost -u mwqtiakilx_stormscout -p'hUnva2-qusvot-cekpuf' mwqtiakilx_stormscout -e 'SHOW COLUMNS FROM advisories WHERE Field IN (\"vtec_event_id\", \"vtec_action\");'"

# Check for remaining duplicates
ssh stormscout "mysql -h localhost -u mwqtiakilx_stormscout -p'hUnva2-qusvot-cekpuf' mwqtiakilx_stormscout -e 'SELECT vtec_event_id, site_id, advisory_type, COUNT(*) as count FROM advisories WHERE status=\"active\" AND vtec_event_id IS NOT NULL GROUP BY vtec_event_id, site_id, advisory_type HAVING count > 1;'"

# Check site 219 (should be ~19 alerts instead of 30)
ssh stormscout "mysql -h localhost -u mwqtiakilx_stormscout -p'hUnva2-qusvot-cekpuf' mwqtiakilx_stormscout -e 'SELECT COUNT(*) as count FROM advisories WHERE site_id = 219 AND status=\"active\";'"

# View action code distribution
ssh stormscout "mysql -h localhost -u mwqtiakilx_stormscout -p'hUnva2-qusvot-cekpuf' mwqtiakilx_stormscout -e 'SELECT vtec_action, COUNT(*) as count FROM advisories WHERE status=\"active\" GROUP BY vtec_action;'"
```
...
```bash
# Rollback database
ssh stormscout "mysql -h localhost -u mwqtiakilx_stormscout -p'hUnva2-qusvot-cekpuf' mwqtiakilx_stormscout < ~/rollback-vtec-event-id-and-action.sql"
...
ssh stormscout "mysql -h localhost -u mwqtiakilx_stormscout -p'hUnva2-qusvot-cekpuf' mwqtiakilx_stormscout -e 'SELECT COUNT(*) FROM (SELECT vtec_event_id FROM advisories WHERE status=\"active\" AND vtec_event_id IS NOT NULL GROUP BY vtec_event_id, site_id HAVING COUNT(*) > 1) as dups;'"
```
