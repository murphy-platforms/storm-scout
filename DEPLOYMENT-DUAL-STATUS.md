...
mysqldump --defaults-extra-file=<(echo -e "[client]\nuser=mwqtiakilx_stormuser\npassword=7GEq^LiZiK=Z\nhost=localhost") \
  mwqtiakilx_stormscout > ~/backups/stormscout_pre_dual_status_$(date +%Y%m%d_%H%M%S).sql
...
mysql --defaults-extra-file=<(echo -e "[client]\nuser=mwqtiakilx_stormuser\npassword=7GEq^LiZiK=Z\nhost=localhost") \
  mwqtiakilx_stormscout -e "DESCRIBE site_status;"

# Check current operational status distribution
mysql --defaults-extra-file=<(echo -e "[client]\nuser=mwqtiakilx_stormuser\npassword=7GEq^LiZiK=Z\nhost=localhost") \
  mwqtiakilx_stormscout -e "SELECT operational_status, COUNT(*) FROM site_status GROUP BY operational_status;"
```
...
mysql --defaults-extra-file=<(echo -e "[client]\nuser=mwqtiakilx_stormuser\npassword=7GEq^LiZiK=Z\nhost=localhost") \
  mwqtiakilx_stormscout < ~/storm-scout/backend/src/data/migrations/add-dual-status.sql
```
...
curl -s https://teammurphy.rocks/api/status/overview | jq '.data | {weather_impact_counts, operational_status_counts}'

# Test new operational status endpoint
curl -s https://teammurphy.rocks/api/operational-status/summary | jq '.data.summary'

# Test sites that need attention
curl -s https://teammurphy.rocks/api/operational-status/summary | jq '.data.needs_attention'
```

#### 4.2 Test Dashboard UI
1. Open https://teammurphy.rocks in browser
...
# Restore backup
mysql --defaults-extra-file=<(echo -e "[client]\nuser=mwqtiakilx_stormuser\npassword=7GEq^LiZiK=Z\nhost=localhost") \
  mwqtiakilx_stormscout < ~/backups/stormscout_pre_dual_status_YYYYMMDD_HHMMSS.sql
...
```bash
curl -X POST https://teammurphy.rocks/api/operational-status/sites/1 \
...
```bash
curl -X POST https://teammurphy.rocks/api/operational-status/bulk-update \
  -H "Content-Type: application/json" \
  -d '{
    "site_ids": [1, 2, 3, 4, 5],
...
curl -s https://teammurphy.rocks/api/operational-status/sites/1 | jq
...
curl -s https://teammurphy.rocks/api/status/overview | jq '.success'
