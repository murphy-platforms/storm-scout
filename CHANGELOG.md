...
- **#105 N+1 query in getAllTrends()** — `advisoryHistory.js` `getAllTrends()` replaced a `DISTINCT office_id` query + `Promise.all(officeIds.map(getTrend))` fan-out (up to N concurrent queries) with a single SQL query fetching all rows in the window followed by O(n) JS grouping by `office_id`
...
### Changed
- **Office Refactor** - Replaced legacy locations with offices identified by 5-digit zip codes; removed all legacy references
...
### Added
- **New Office: Concord, NH** - office added
  - Location: Concord, NH
